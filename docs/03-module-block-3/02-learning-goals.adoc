...
Die Teilnehmer:innen wissen, dass Datenbanken durch ein Datenbankmanagementsystem verwaltet werden, welches beispielsweise Schemaverwaltung, Datenzugriff, Query-Optimizer, Datenverteilung bereitstellt. Ihnen ist bewusst, das Datenbanken vorrangig für die Verwaltung strukturierter Daten verwendet werden. Sie sind sich auch der Problematik einer starken Kopplung der Datenmodelle bei direktem Zugriff auf die Datenbank bewusst. 
...
Die Teilnehmer:innen wissen, das der Zugriff auf die Daten typischerweise durch RESTful APIs erfolgt.

Garantien - Nebenläufigkeit

* Zugriff mittels APIs
